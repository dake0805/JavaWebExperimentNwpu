  ...
  <body>
    ...
    <div class="spittleList">
      ...
      <ul class="spittleList">
    	...
    	每页${paginationSupport.pageSize}条吐槽，  第${paginationSupport.currentPageNo}/${paginationSupport.totalPageCount}页,共${paginationSupport.totalCount}条吐槽
      <c:if test="${paginationSupport.previousPage}">
      	<a href="<c:url value="/spittles?pageNo=${paginationSupport.currentPageNo-1}" />" >上一页</a> 
      </c:if>
      <c:if test="${paginationSupport.nextPage}">
      	<a href="<c:url value="/spittles?pageNo=${paginationSupport.currentPageNo+1}" />" >下一页</a>
      </c:if>
  </body>
</html>